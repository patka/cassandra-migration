 ...
 */
public class DatabaseWithPrefixTest {

    public static final String TABLE_PREFIX = "prefix_";
    @Rule
    ...
    @Before
    public void setUp() {
        database = new Database(cassandra.getCluster(), CassandraJUnitRule.TEST_KEYSPACE, TABLE_PREFIX);
    }
    ...
    public void shouldCreateKeyspaceWhenDatabaseWithoutKeyspaceAndKeyspaceDefinitionGiven() {
        ...
        Database db = new Database(cassandra.getCluster(), keyspace, TABLE_PREFIX);
    ...
    private List<Row> loadMigrations() {
        ...
        ResultSet resultSet = session.execute(new SimpleStatement("SELECT * FROM " + TABLE_PREFIX + "schema_migration;"));
